        ...
        <>
            <div className="w-full">
                ...
                <div className="flex justify-end items-center mb-8">
                    <div className="flex flex-col sm:items-end space-y-4 w-full">
                        <div className="flex flex-col justify-end sm:flex-row sm:items-center gap-2 sm:gap-4 w-full">
                        ...
                        <TypographyH3>{invoiceData.invoiceNumber}</TypographyH3>
                    </div>
                </div>

                {/* Invoice content */}
                <div ref={invoiceRef} className="text-xs sm:text-sm sm:w-[850px]">
                    {/* Header */}
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start my-6 gap-4">
                        <div>
                        ...
                        <div className="text-right">
                            ...
                            <p><span className="font-semibold">PO/SO Number:</span> {invoiceData.poNumber}</p>
                            <p><span className="font-semibold">Invoice Date:</span> {invoiceData.invoiceDate}</p>
                            <p><span className="font-semibold">Payment Due:</span> {invoiceData.paymentDate}</p>
                            <p><span className="font-semibold">Due Date:</span> {invoiceData.dueDate}</p>
                            <p className="font-semibold mt-2 sm:text-base text-primary text-sm">Amount Due (INR): ₹{invoiceData.amountDue}</p>
                        </div>
                    </div>
                    ...
                    {/* Table Header */}
                    <div className="grid grid-cols-6 bg-primary text-white text-sm py-2 px-3">
                        <div>Items</div>
                        <div>Description</div>
                        <div>Quantity</div>
                        <div>Price</div>
                    ...
                    {invoiceData.items.map((item, i) => (
                        <div key={i} className="grid grid-cols-6 border-b py-2 px-3 items-start">
                            <div>{item.name}</div>
